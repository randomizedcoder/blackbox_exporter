...
func probeICMPNative(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, logger *slog.Logger) (success bool) {
	...
	if len(module.ICMP.SourceIPAddress) > 0 {
		...
		logger.Debug("Using source address", "srcIP", srcIP)
	}
	...
	logger.Debug("Creating ICMP packet", "seq", body.Seq, "id", body.ID)
	wm := icmp.Message{
		Type: requestType,
		Code: 0,
		Body: body,
	...
	logger.Debug("Writing out packet")
	rttStart := time.Now()
	...
	logger.Debug("Waiting for reply packets")
	for {
		var n int
		var peer net.Addr
		var err error
